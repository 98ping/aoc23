package ltd.matrixstudios.helpers.sint

import ltd.matrixstudios.helpers.divBy
...
internal fun <T> windowedIterator(
	iterator: Iterator<T>,
	size: Sint,
	step: Sint,
	...
	return iterator<List<T>> {
		...
		if (gap >= 0) {
			...
			for (e in iterator) {
				...
				if (buffer.isFull()) {
					if (buffer.size < size) {
						buffer = buffer.expanded(maxCapacity = size.i); continue
					}
...
private class RingBuffer<T>(private val buffer: Array<Any?>, filledSize: Sint) : AbstractList<T>(), RandomAccess {
	...
	fun expanded(maxCapacity: Int): RingBuffer<T> {
		...
		return ltd.matrixstudios.helpers.sint.RingBuffer(newBuffer, size.s)
	}
